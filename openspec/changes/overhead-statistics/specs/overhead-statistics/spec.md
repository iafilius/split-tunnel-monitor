...
The system SHALL establish a session baseline p50 overhead from the first N valid samples, where N is configurable via `--overhead-baseline-samples` (default: 30). Once set, the baseline SHALL remain fixed for the duration of the session.
...
- **WHEN** the network interface changes mid-session and the rolling window is active
