...
- [ ] 21.1 `git pull` this branch first — this picks up the `.gitignore` negation for `docs/traces/**/*.log` (task 20.1) before touching that folder. Without it, `git add docs/traces/*.log` on an older checkout would silently do nothing (the repo's blanket `*.log` rule would still apply).
- [ ] 21.2 Locate the raw `--logfile` output already captured for Trace 1d, Trace 1e, and Trace 1f (task 15.4) — check wherever `split-tunnel-monitor`/`ping_checker.py` wrote them on this machine (or the `ping -c 120 -i 0.2` output for 1f if it was only piped to a file). If any of the three no longer exist on disk, note that explicitly here rather than reconstructing/approximating one from the pasted excerpt.
- [ ] 21.3 Commit whichever of the three are still available to `docs/traces/`, following the naming convention from task 20.2: `trace-1d-m3-battery-lpm-clean-n120.log`, `trace-1e-m3-ac-clean-n120.log`, `trace-1f-m3-highfreq-ping-n120.log` (adjust names to match what each trace actually is if these don't fit exactly). Link each committed file from its corresponding `### Trace 1d/1e/1f: ...` entry in Section 5, the same way Trace 3d/3e already do.
- [ ] 21.4 Before making any further edits to Section 2, Section 3.4/3.5, or Section 6: re-read `design.md` Decision 16 and 17 and the two spec requirements they codify ("Quantitative Claims Must Cite a Real, Checkable Capture" and "Raw Trace Evidence Is Committed to the Repository") in `specs/wifi-latency-forensics/spec.md`. This is the second time absolute-certainty language ("conclusively proven", "100% identical") crept back into Section 6's causality paragraph after being softened once already (task 12.4, then reverted, then re-fixed in task 19.2) — check any new causal-conclusion prose against Decision 16 before committing it.
- [ ] 21.5 Run `openspec validate --all` and `pytest` after committing the trace files and reviewing the two decisions, and report back (via tasks.md, since that's the shared channel) whether all three M3 raw logs were found and committed, or which ones weren't recoverable.
