...
- [ ] 15.3 Re-capture Trace 3b (Corporate M2 Pro, AC Power, Zscaler Bypassed) at `--count 120` — requires disabling Zscaler Internet Access in the ZCC UI first (manual toggle, not automatable from here).
- [ ] 15.4 Re-capture Trace 1a (Personal M3, Battery + Low Power Mode) and Trace 1b (Personal M3, AC Power) at `--count 120` — on the M3 laptop, same as the original captures.
- [ ] 15.5 Re-run the Section 6 "Statistical Power & Confidence" worked example against the new n≈120 counts (recompute z-scores) and update the observation text and Section 2 table percentages to the re-captured values.
- [ ] 15.6 Run `openspec validate --all` and `pytest` after the doc is updated with the new traces.
