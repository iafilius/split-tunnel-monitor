...
**Implemented as a single shared helper, `merge_egress_result(current_egress, fresh_eg) -> dict`, not inline duplication.** Both call sites are closures nested inside `main()`, which has no existing test seam (this codebase has no tests exercising `main()`'s internals at all). Extracting the merge logic into one module-level, directly-importable function makes it independently unit-testable without needing to instantiate or mock `main()`'s surrounding state — while keeping both call sites' behavior identical to the inline version originally sketched above. This is a structural refinement over the initial sketch, not a behavior change.

**Compute change-detection against the state as it exists *after* the merge decision, not before.** Since a sub-part that fails to resolve is now never written, the existing `direct_changed`/`tunneled_changed` comparisons (which read `old_*` from `current_egress` *before* any mutation this call) remain correct without modification — they were already comparing against the right baseline; it was only the *subsequent* unconditional overwrite that discarded good data out from under them for the *next* call. No change needed to the comparison logic itself, only to what happens after it. One refinement made during implementation: `tunneled_changed` now also requires the new fingerprint to be non-empty (`bool(new_tunneled_fp and new_tunneled_fp != old_tunneled_fp)`), mirroring the existing guard already used for `direct_changed` — an empty tunneled result is a discovery failure, not "the tunnel became pending," so it must not trigger a log line either (this was the exact cause of the spurious `Tunnel: Pending / Offline` line seen in the original investigation).
